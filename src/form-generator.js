// --- Form Item Types ---
// Label
const label = ({ id = '', display = '', target = '' }) => `<label id="${id}" for="${target}">${display}</label>`;
...
// Input (Text & Submit)
const input = ({ type = '', id = '', display = '' }) => `<input type="${type}" id="${id}" value="${display}" placeholder="${display}">`;

// --- Form Generator ---
const generateForm = formItems => new Promise((resolve, reject) => {
  ...
  return resolve(`<form>${formBody}</form>`);
});
